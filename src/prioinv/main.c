...
void (*vOLEDImageDraw)(const unsigned char*, uint32_t, uint32_t, uint32_t,
		uint32_t) = NULL;
...
#define SISTEMA_ACTUAL 8

xTask tasksSistema1[3] = { { 1000, 5000 }, { 1000, 5000 }, { 2000, 8000 } };
xTask tasksSistema2[3] = { { 1000, 4000 }, { 1000, 8000 }, { 2000, 9000 } };
xTask tasksSistema3[4] = { { 1000, 4000 }, { 1000, 6000 }, { 1000, 8000 }, {
		3000, 13000 } };
xTask tasksSistema4[4] = { { 1000, 4000 }, { 2000, 7000 }, { 1000, 12000 }, {
		2000, 14000 } };
xTask tasksSistema5[5] = { { 1000, 5000 }, { 1000, 9000 }, { 1000, 10000 }, {
		2000, 15000 }, { 2000, 16000 } };
xTask tasksSistema6[5] = { { 1000, 6000 }, { 1000, 8000 }, { 2000, 11000 }, {
		1000, 15000 }, { 2000, 17000 } };
xTask tasksSistema7[4] = { { 1000, 4000 }, { 1000, 7000 }, { 2000, 10000 }, {
		2000, 14000 } };
xTask tasksSistema8[4] = { { 1000, 4000 }, { 1000, 8000 }, { 2000, 10000 }, {
		2000, 14000 } };
xTask tasksSistema9[5] = { { 1000, 5000 }, { 1000, 8000 }, { 2000, 12000 }, {
		1000, 15000 }, { 1000, 16000 } };
xTask tasksSistema10[5] = { { 1000, 5000 }, { 1000, 8000 }, { 1000, 12000 }, {
		1000, 13000 }, { 2000, 16000 } };

void crearTareasPorNumeroSistema(int numeroSistema) {
	xTask *tasksSistema = NULL;
	int cantTareas = 0;

	if (numeroSistema == 1) {
		tasksSistema = tasksSistema1;
		cantTareas = 3;
	} else if (numeroSistema == 2) {
		tasksSistema = tasksSistema2;
		cantTareas = 3;
	} else if (numeroSistema == 3) {
		tasksSistema = tasksSistema3;
		cantTareas = 4;
	} else if (numeroSistema == 4) {
		tasksSistema = tasksSistema4;
		cantTareas = 4;
	} else if (numeroSistema == 5) {
		tasksSistema = tasksSistema5;
		cantTareas = 5;
	} else if (numeroSistema == 6) {
		tasksSistema = tasksSistema6;
		cantTareas = 5;
	} else if (numeroSistema == 7) {
		tasksSistema = tasksSistema7;
		cantTareas = 4;
	} else if (numeroSistema == 8) {
		tasksSistema = tasksSistema8;
		cantTareas = 4;
	} else if (numeroSistema == 9) {
		tasksSistema = tasksSistema9;
		cantTareas = 5;
	} else if (numeroSistema == 10) {
		tasksSistema = tasksSistema10;
		cantTareas = 5;
	}

	char tareaInfo[150];
	for (int tareaIndice = 0; tareaIndice < cantTareas; tareaIndice++) {

		sprintf(tareaInfo, "T%d - %u", tareaIndice + 1,
				tasksSistema[tareaIndice].period);
//		sprintf(tareaInfo, "T%d - %d", tareaIndice + 1, sizeof(tasksSistema)-1);

		xTaskCreate(tarea, tareaInfo, configMINIMAL_STACK_SIZE + 50,
				(void*) &tasksSistema[tareaIndice],
				configMAX_PRIORITIES - (tareaIndice + 1), NULL);
	}
}
...
int main(void) {
	...
	crearTareasPorNumeroSistema(SISTEMA_ACTUAL);
...
void prvTask(void *pvParameters) {
	...
	for (;;) {
		sprintf(cMessage, "%s - %u\n\r", pcTaskGetTaskName( NULL),
				uxReleaseCount);

		prvPrintString(cMessage);

		vBusyWait(task->wcet - busyWaitDelayPrint);
