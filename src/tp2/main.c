...
void crearTareasPorNumeroSistema(int numeroSistema) {
	...
	char tareaInfo[150];
	for (int tareaIndice = 0; tareaIndice < cantTareas;
			tareaIndice++) {

		sprintf(tareaInfo, "T%d - %u", tareaIndice + 1, tasksSistema[tareaIndice].period);
//		sprintf(tareaInfo, "T%d - %d", tareaIndice + 1, sizeof(tasksSistema)-1);

		xTaskCreate(prvTask, tareaInfo, configMINIMAL_STACK_SIZE + 50,
